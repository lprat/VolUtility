...
def run_plugin(session_id, plugin_id, pid=None, plugin_options=None):
    ...
    if session_id and plugin_id:
        ...
        if plugin_name in ["yarascan"]:
            plugin_options={
            'YARA_FILE': '/opt/VolUtility/yararules/malware_rules.yar',
            'CASE': None,
            'ALL': True,
            'WIDE': None,
            'SIZE': 512,
        ...
        if results:
            #check if columns data exist and get position row, check if data contains hex
            if 'Data' in results['columns'] and results['rows'] and results['rows'][0][results['columns'].index('Data')]:
                try:
                    logger.debug('Running Plugin with data: {0}'.format(plugin_name))
                    int(results['rows'][0][results['columns'].index('Data')],16)
                    logger.debug('Running Plugin with datahex: {0}'.format(plugin_name))
                    results['columns'].append('Data_ascii')
                    results['columns'].append('Data_disa')
                    pos_data=results['columns'].index('Data')
                    for row in results['rows']:
                        try:
                            row.append(string_clean_hex_noprint(row[pos_data].decode('hex')))
                        except:
                            row.append("Error to translate Data hexto Ascii")
                        md = Cs(CS_ARCH_X86, CS_MODE_32)
                        if 'x64' in session['session_profile']:
                              md = Cs(CS_ARCH_X86, CS_MODE_64)
                        addr_ptr=0x100000
                        if 'Address' in results['columns'] and results['rows'][0][results['columns'].index('Address')] and isinstance(results['rows'][0][results['columns'].index('Address')],(int,long)):
                           addr_ptr=results['rows'][0][results['columns'].index('Address')]
                        dissa=''
                        try:
                            for i in md.disasm(row[pos_data].decode('hex'), addr_ptr):
                                dissa+="0x%x:\t%s\t%s\n<br>" %(i.address, i.mnemonic, i.op_str)
                            row.append(dissa)
                        except:
                            row.append("Error to disassemble Data hex")
                        row[pos_data]=data_clean(row[pos_data])
                except:
                    pass
            # Start Counting
            counter = 1
    ...
    if command == 'pluginresults':
        ...
        if 'pagination' in request.POST:
            ...
            for row in output[start:start+length]:
                tmp=[]
                for cell in row:
                    if isinstance(cell, basestring) and '<br>' in cell:
                        tmp.append('<a class="readOnlyRowsToggle" href="#read">Display</a><div  class="readOnlyRow">'+cell+'</div><script>$(document).ready(function() { $(".readOnlyRow").hide(); $(\'.readOnlyRowsToggle\').click(function () { $(this).next(".readOnlyRow").toggle($(this).next(".readOnlyRow").css(\'display\') == \'none\'); });});</script>')
                    else:
                        tmp.append(cell)
                paged_data.append(tmp)
            datatables = {
        ...
        # Else return standard 25 rows
            ...
            tl_order=[]
            if 'timeliner' in plugin_results['plugin_name']:
                tl_stat={}
                for tlrow in output:
                    if tlrow[2] in tl_stat:
                        tl_stat[tlrow[2]]+=1
                    else:
                        tl_stat[tlrow[2]]=1
                tmptl = sorted(tl_stat.items(), key=lambda kv: kv[1])
                for k,v in tmptl:
                    tl_order.append([k,v])
            yara_sortsig=[]
            yara_sortproc=[]
            ...
            yara_stat_byproc={}
            yara_order=[]
            if 'yarascan' in plugin_results['plugin_name']:
                for yarow in output:
                    if yarow[1] in yara_stat_bysig:
                        yara_stat_bysig[yarow[1]]['total']+=1
                        if yarow[2] in yara_stat_bysig[yarow[1]]:
                            yara_stat_bysig[yarow[1]][yarow[2]]+=1
                        else:
                            yara_stat_bysig[yarow[1]][yarow[2]]=1
                    else:
                        yara_stat_bysig[yarow[1]]={}
                        yara_stat_bysig[yarow[1]]['total']=1
                        yara_stat_bysig[yarow[1]][yarow[2]]=1
                    if yarow[2] in yara_stat_byproc:
                        yara_stat_byproc[yarow[2]]['total']+=1
                        if yarow[1] in yara_stat_byproc[yarow[2]]:
                            yara_stat_byproc[yarow[2]][yarow[1]]+=1
                        else:
                            yara_stat_byproc[yarow[2]][yarow[1]]=1
                    else:
                        yara_stat_byproc[yarow[2]]={}
                        yara_stat_byproc[yarow[2]]['total']=1
                        yara_stat_byproc[yarow[2]][yarow[1]]=1
            if yara_stat_bysig:
                yara_order=sorted(yara_stat_bysig, key=lambda xs: (yara_stat_bysig[xs]['total']))
                for k in yara_order:
                    for kx,vx in yara_stat_bysig[k].items():
                        if not kx == 'total':
                            yara_sortsig.append([k+'('+str(yara_stat_bysig[k]['total'])+')',kx,vx])
            if yara_stat_byproc:
                yara_order=sorted(yara_stat_byproc, key=lambda xs: (yara_stat_byproc[xs]['total']))
                for k in yara_order:
                    for kx,vx in yara_stat_byproc[k].items():
                        if not kx == 'total':
                            yara_sortproc.append([k+'('+str(yara_stat_byproc[k]['total'])+')',kx,vx])

            rendered_data = render(request, 'plugin_output.html', {'plugin_results': plugin_results['plugin_output'],
                                                          'plugin_id': plugin_id,
                                                          'bookmarks': bookmarks,
                                                          'resultcount': resultcount,
                                                          'plugin_name': plugin_results['plugin_name'], 'yara_sortsig': yara_sortsig, 'yara_sortproc': yara_sortproc, 'tl_order': tl_order})
            if rendered_data.status_code == 200:
                return_data = rendered_data.content
            else:
                return_data = str(rendered_data.status_code)
