...
class RunVol:
    ...
    def get_text_psinfo(self, plugin_class):
        """
        ...
        out_rows=[]
        rows=plugin_data.split('---------------------------------------------------')
        for row in rows:
            cellx=row.strip().split('\n')
            pname=""
            ppname=""
            if len(cellx) > 3:
                pname=cellx[1].strip().replace('Process: ','').replace('PID: ','[')+"]"
                ppname=cellx[2].strip().replace('Parent Process: ','').replace('PPID: ','[')+"]"
            out_rows.append([pname,ppname,'<pre>\n{0}\n</pre>'.format(row.strip())])
        # Return a json object from our string so it matches the json output.
        # Also going to drop in pre tags here
        return {'columns': ['Process','Parent Process','Informations'], 'rows': out_rows}
    ...
    def run_plugin(self, plugin_name, pid=None, dump_dir=None, plugin_options=None, hive_offset=None, output_style="json"):
        ...
        if plugin_name in cmds.keys():
            ...
            elif plugin_name == 'psinfo':
                output_data = self.get_text_psinfo(command)
                return output_data
